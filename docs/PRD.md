...
| Bounded connector pull | `core/connectors/s3_connector.py:_pull_sync` is bounded to **one date folder per build** AND **200 docs per build** within that folder (`_PULL_CAP_PER_BUILD = 200`). Picker uses `folder_date >= wm.date()` so the bounded pull re-picks the same date until drained. When the folder has more than the cap → returns first 200 sorted by `received_at` and the wm naturally lands mid-day. When the folder has `<= cap` docs AND another folder exists ahead in window → connector appends an in-band `{_eod_marker: True, received_at: start-of-next-day-iso}` doc; the builder splits markers out before persist (they carry no `document_id`) but folds their `received_at` into `wm_to` so the watermark advances past the drained folder. A 2000-doc day drains in 10 builds instead of 1 that OOMs. Builder also surfaces `watermark_from` + `watermark_to` on stats so the catch-up loop can detect marker-only progress and continue. |
...
| Golden-record backfill | `core/aggregation/golden_record_builder.py` provides `rebuild_one(pg, redis, application_id, tenant_id)` — the per-application orchestrator that writes all five golden-record tables (`income_profiles`, `credit_profiles`, `applicant_identity_xref`, `entity_states`, `entity_state_events`) from already-indexed docs. Every write is idempotent: profiles are DELETE+INSERT, xref is UPSERT DO NOTHING, entity_states is UPSERT DO UPDATE (full row replacement except `legacy_ids` which JSONB-merges). Composes the v4 single-row-per-application `entity_states` shape (borrower + co_borrowers + property + loan_terms JSONB + indexed columns: `mid_credit_score = min(borrowers)`, `qualifying_monthly`, `combined_monthly_income`, `loan_amount`/`interest_rate` with URLA→RATE_LOCK fallback, `ltv = loan/min(appraised,purchase)*100`, MI by LTV band (>95% 1.10%, 90-95% 0.80%, 85-90% 0.52%, 80-85% 0.32%, ≤80% 0), `piti_monthly` via amortization+tax+HOI+MI, `dti_front/back`, `total_liquid_assets`, 12-flag verifications, `completeness_pct`, `status`). Replaces the broken `entity_state_builder.upsert_all_entities` orchestrator (signature mismatch that silently no-op'd in prod for months — entity_states empty despite 261k indexed docs). Wired into `_run_assembly` so future uploads automatically keep all 5 tables current. |
| Backfill orchestrator API | `POST /admin/rebuild-golden-records?force=true` (admin-scoped) spawns a background asyncio task that drains every application in tenant via `rebuild_one`. Streams `application_id ASC` via asyncpg server-side cursor (memory-safe on 9k applications, prefetch=500). Commits progress per-application to a new singleton `golden_record_backfill_state` table (`last_completed_application_id` + counts + errors + started_at + completed_at). On crash the next call resumes from the cursor; `?force=true` resets and re-UPSERTs every row. Returns 202 immediately. `GET /admin/rebuild-golden-records/status` returns `{completed, total, status, last_completed, errors, started_at, completed_at, elapsed_seconds, tenant_id}` — pollable every few seconds. Caps the errors array at 100 entries on the watermark row so a runaway loop can't bloat it. |
...
2. `python -m pytest tests/ --ignore=tests/integration -q` reports
   `362 passed, 2 skipped` (live tests skipped without API key) on a
   fresh checkout. Integration + smoke add 11 more for `373 green`.
