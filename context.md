...
- Branch: `main`, all committed and pushed to origin (`https://github.com/krishnamami/edms-simulator`). The most recent six sessions added the three Decision-OS-facing API interfaces (Reports + Exports + Application), client-grade OpenAPI/Swagger surface, full multi-tenancy, unified per-API-key rate limiting, async webhook outbox, and auto-migration on startup.
- Tests: **332 passing, 2 skipped** (live-API tests gated on `ANTHROPIC_API_KEY`). +3 integration + 8 smoke = 343 green.
