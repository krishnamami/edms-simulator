...
- Branch: `main`, all committed and pushed to origin (`https://github.com/krishnamami/edms-simulator`). The most recent ten sessions added the three Decision-OS-facing API interfaces (Reports + Exports + Application), client-grade OpenAPI/Swagger surface, full multi-tenancy, unified per-API-key rate limiting, async webhook outbox, schema auto-migration on startup, the **incremental knowledge-graph backtest harness** (S3 connector + builder + EOD snapshots + 50-day replay) with both inproc and HTTP-driven modes, **upload-path write-through into `entity_states` for all four lending entities** (borrower / co-borrower / property / loan_terms), the **channel-segmented v2 simulation pipeline** (10-loan × 50-day × 9-channel generator + connector dispatch + builder los_id resolution), the **v4 scale-simulation pipeline** (9000-loan × 22-profile generator + bounded connector pulls + async catch-up + zombie-hardened scheduler), and most recently the **golden-record backfill orchestrator** that fixes a long-standing entity_states write-path mismatch and adds POST /admin/rebuild-golden-records.
- Tests: **362 passing, 2 skipped** (live-API tests gated on `ANTHROPIC_API_KEY`). +3 integration + 8 smoke = 373 green.
- **Golden-record backfill orchestrator + entity_states write-path fix** (commits `06ffab9` → `3737719`). Five tables were empty in prod (`entity_states`, `entity_state_events`, `income_profiles`, `credit_profiles`, `applicant_identity_xref`) despite 261k docs in `document_index` and 312k edges in `document_relationships`. Root cause: `core/aggregation/entity_state_builder.py:upsert_all_entities` called `pg.upsert_entity_state(entity_id=, entity_type=, application_id=, state=, …)` against an actual PG signature of `(application_id, state_data, tenant_id)` — every call landed in the per-entity `except Exception` clause; warnings were the only signal, and `_run_assembly` silently no-op'd the write-through. New module `core/aggregation/golden_record_builder.py` composes the row the schema actually wants. **`rebuild_one(pg, redis, application_id, tenant_id)`** runs both assemblers in the same order as the upload path (credit first so income reads it), writes `income_profiles` + `credit_profiles` (`save_*_profile` is DELETE+INSERT — idempotent), upserts one `applicant_identity_xref` per `(applicant_id, source_channel, source_document_id)` triple (UPSERT DO NOTHING), composes a `state_data` dict with `borrower` / `co_borrowers` / `property` / `loan_terms` JSONB and all the indexed cols (`mid_credit_score` = min of borrowers, `qualifying_monthly`, `combined_monthly_income`, `loan_amount`/`interest_rate` with URLA→RATE_LOCK fallback chain, `ltv = loan/min(appraised,purchase)*100`, MI by LTV band (>95% 1.10%, 90-95% 0.80%, 85-90% 0.52%, 80-85% 0.32%, ≤80% 0), `piti_monthly` via amortization+tax+HOI+MI, `dti_front/back`, `total_liquid_assets` from ending_balance, 12-flag verifications, `completeness_pct`, `status`), UPSERTs via the correct signature, and logs an `entity_state_events` row. **`run_backfill(...)`** drains every application via an asyncpg server-side cursor (`ORDER BY application_id ASC`, prefetch=500 — memory-safe on the 9k corpus), commits progress per-application to a new singleton `golden_record_backfill_state` table, resumes from `last_completed_application_id` on restart, and caps errors at 100 entries on the row. New endpoints: **`POST /admin/rebuild-golden-records?force=true`** returns 202 + spawns the bg task; **`GET /admin/rebuild-golden-records/status`** returns `{completed, total, status, last_completed, errors, started_at, completed_at, elapsed_seconds, tenant_id}`. Wired into `_run_assembly` as `_compose_and_upsert_entity_state(...)` (without re-running assemblers since the upload path already did) so future uploads keep all 5 tables current. Production crash on first run surfaced an asyncpg quirk: ISO timestamp strings on TIMESTAMPTZ columns trigger `DataError: invalid input for query argument $7 (expected datetime, got 'str')` — fixed in `3737719` by storing real `datetime` objects in state + adding a `_as_dt()` coercion at the write boundary. Tests: 359 → 362 (3 new in `tests/core/aggregation/test_golden_record_builder.py`: writes_to_all_five_tables, idempotent_on_re_run, skips_unknown_application).
- **v4.4 — per-build doc cap (200) + EOD marker for cross-day advancement** (commit `8b1832c`). A scale-corpus day folder can carry 2k+ docs; loading them all into memory at once OOM'd the Fargate task even at 4 GiB (boto3 listing + JSON parsing + indexer state + extractor concurrency overlap). Connector now caps each pull at `_PULL_CAP_PER_BUILD = 200`. Picker changed `>` → `>=` so the bounded pull re-picks the same date until drained. When a folder has more than the cap, returns first 200 sorted by `received_at`; wm lands mid-day; next build re-picks the same folder. When a folder has `<= cap` docs AND another folder exists ahead in the window, the connector appends an in-band `{_eod_marker: True, received_at: start-of-next-day-iso}` doc; the builder splits markers out before persist (no `document_id`) but folds their `received_at` into `wm_to` so the watermark advances past the drained folder. Builder also now surfaces `watermark_from` + `watermark_to` on stats (these were missing entirely — that's why `current_watermark` was `null` in the catch-up status). Catch-up break condition extended: `pulled == 0 && apps == 0 && !wm_advanced` (a marker-only pull is still forward progress). A 2000-doc day now drains in 10 builds instead of 1 that crashes. Watermark verified advancing `2025-12-31` → `2026-01-21T17:15Z` end-to-end. 3 new tests (cap returns N + no marker; folder drained with next folder → marker emitted; folder drained as last → no marker). 356 → 359 green.
- **ECS sizing churn across the session.** Started at 0.5 vCPU / 1 GiB → `d8a6ba4` 1 vCPU / 2 GiB (halve per-build wall clock on scale corpus) → `9dffded` 1 vCPU / 4 GiB (3GB→4GB headroom for bounded-build catchup, but the underlying RDS `db.t3.micro` was already saturated — SSL handshakes timing out at 60s, /health stalling, ECS marking tasks unhealthy → 502s; user upgraded RDS out-of-band) → `65b776c` 0.5 vCPU / 1 GiB (scale sim done, normal API serving doesn't need the headroom). Out-of-band CLI overrides: rev 101 (1024/4096) and rev 105 (512/1024); `bdfa5bd` mirrored rev 101 in git, so git can drift from live until the next `task_definition.json` edit syncs them.
- **v4 scale-simulation pipeline — 9000 loans × 22 profiles × 60 days, async catch-up, zombie-hardened scheduler** (commits `61eebfe` → `d8a6ba4`). `scripts/generate_scale_simulation.py` writes ~338k objects across 22 origination profiles (Conv/FHA/VA/USDA/Jumbo/NonQM/HELOC/Construction/Reverse/Bridge/Renovation/etc.) into `s3://edms-simulator-loans/s3_scale_simulation/{date}/{loan_origination|post_application/<channel>}/...` with ±2-day per-loan jitter; 100 of the 9k loans get rendered binary `.pdf` evidence (others stay JSON-only to keep generation tractable). `scripts/verify_scale_results.py` runs 15 production-readiness checks (counts, watermark progression, edges, snapshots, completeness, `extraction_method` distribution, contradicts edges) against the deployed API. **Connector bounded to one date folder per build** (`core/connectors/s3_connector.py:_pull_sync`) — fresh-from-epoch listing 338k objects in one shot was ALB-timing-out the build, so the pull now sorts every date folder, picks only the FIRST strictly after `wm.date()`, and returns `[]` when nothing is left. The catch-up loop drives this forward day-by-day; existing `documents_pulled == 0 && apps == 0` termination handles end-of-corpus. **`/scheduler/trigger` and `/scheduler/catch-up` both async background tasks** with the catch-up state persisted at Redis key `catchup:state` so any of the 2+ replicas answering `GET /scheduler/catch-up/status` returns the same view (a POST hits replica A, a GET on replica B used to report `idle` while A was draining — the in-process module dict had to move). **Catch-up break condition** evolved across the session: started as `documents_new == 0` (stalled when a folder of all-events with 0 docs landed); switched to `(new == 0 && apps == 0)` (stalled when an already-indexed folder showed `new == 0` even though pulled was non-zero); landed on `documents_pulled == 0 && applications_created == 0` (only stops when the connector found ABSOLUTELY NOTHING). **Catch-up zombie hardening** — the bg task could crash silently and leave Redis at `status='running'` forever; now `last_heartbeat` is written before AND after every build, the POST handler treats `running + heartbeat>60s` as a zombie and resets, `?force=true` is the operator escape hatch, per-build try/except with `error_count >= 3` → `stopped_reason='too_many_errors'`, and a `job_id` self-check inside the bg loop makes the loser of a force-race exit silently rather than stomping the new run's counters. POST also accepts `max_builds` via JSON body. GET surfaces `heartbeat_age_s` so operators don't have to subtract timestamps. **15-minute cron** — `config/schedule.yaml` collapsed three hard-coded cron times into a single `*/15 * * * *` so backlogs drain continuously instead of waiting for the next named slot. **`/admin/reset`** endpoint nukes prod data for re-runs (route lives under `api/admin.py` mounted with `prefix=/admin`, so the decorator is `/reset` not `/admin/reset`). **ECS task bumped to 1 vCPU / 2 GiB** (`d8a6ba4`) — `0.5 vCPU / 1 GiB` was throttling each bounded build to ~150–200s on the scale corpus; doubling halves per-build wall clock. Live drain verified: watermark advanced `2025-12-31` → `2026-01-21T17:15Z` past the prior cap, `documents_pulled` typically ~2× `documents_processed` (indexer correctly de-duplicating against prior runs), `applications_created` accumulates correctly off `loan_origination/` events. **`task_definition.json` `S3_SIMULATION_SOURCE` → `s3://edms-simulator-loans/s3_scale_simulation`** (`d392cc3`). Tests: 353 → 356 across the session, all green.
- **v3 simulation pipeline — two-stage connector dispatch + idempotent `create_application_from_event`** (commit `ba5e749`). The v3 layout has `{date}/loan_origination/` and `{date}/post_application/{channel}/` instead of v2's flat channel sub-folders. Connector adds: (a) `_iter_channels` v3 detection that yields `loan_origination/` FIRST so the builder's new step 2.0 creates apps before the rest of the day's docs need los_id resolution, then walks `post_application/{channel}/` for ~50 known channels (`_INDIVIDUAL_JSON_CHANNELS` expanded with `los_openclose`, all 5 employer payroll providers, 7 vendors, IRS/SSA/VA, servicer, compliance, LO notes; `_META_PAIR_CHANNELS` adds 2 AMCs, 3 title companies, 5 insurance carriers, closing_agent, hoa_management, condo_project, conditions_response, corrections, attorney_legal; `_RAW_SCAN_CHANNELS` adds 4 manual-drop variants); (b) new format families: `_CSV_CHANNELS` (BytePro via DictReader, one doc per row), `_XML_CHANNELS` (MISMO 3.4 via ET local-name matching of LoanIdentifier/BaseLoanAmount/NoteRatePercent/etc.), `_APPLICATION_EVENT_CHANNELS` (loan_origination tagged `event_type='loan_application_submitted'`). Builder step 2.0 calls `pg.create_application_from_event` for each event doc (idempotent on `los_id`), then drops events from `new_docs` so they never hit the persist gate. Every `upsert_entity_state` now passes a `legacy_ids` accumulator (encompass IDs from the event + every `source_document_id` from docs that touched the applicant this tick) — PG-side JSONB `||` merge keeps the union over time. `save_document` writes new `source_document_id` + `source_channel` columns. Schema additions are **strictly additive** (`ALTER TABLE … ADD COLUMN IF NOT EXISTS`) — the spec literally said `DROP TABLE` in `schema.sql` but that auto-applies on every ECS rolling deploy and would wipe prod data; the destructive recipe lives in `scripts/reset_v3_data.sql` for ops to run once. `config/schedule.yaml` + `task_definition.json` `S3_SIMULATION_SOURCE` → `s3_simulation_v3`. v2 + v1 connector paths preserved (verified by 7 v2 tests staying green).
- **AI-Vision field hints expanded 22 → 56 doc types** (`_EXPECTED_FIELDS` in `core/documents/extractors/claude_extractor.py`). Previously W-2, paystub, credit report, and bank statement had no hints — Claude fell through to the generic "Extract all visible fields" prompt and accuracy collapsed (~50% for W-2, ~20% for multi-page bank stmts where the running ledger distracts from the summary). Added entries for: W2_CURRENT/PRIOR, PAYSTUB_CURRENT, PAYSTUB, CREDIT_REPORT (mid + tri-merge scores, tradeline counts, derogatories), BANK_STATEMENT_M1/M2/M3 (M1 has the richest hint with `nsf_count` / `largest_deposit` / period bounds; M2/M3 trim down so the model focuses on summary fields most likely to land within the `AI_EXTRACTION_MAX_PAGES=3` window), APPRAISAL_URAR/_1073/_UPDATE, TITLE_COMMITMENT/INSURANCE, HOI_BINDER/HO6, FLOOD_CERT, PROPERTY_TAX_BILL, VOE_TWN, DRIVERS_LICENSE, SSN_VALIDATION, OFAC_CHECK, AUS_DU_FINDINGS, SSA_AWARD_LETTER, PENSION_LETTER, HOA_CERT, CONDO_QUESTIONNAIRE, SURVEY, WDO_REPORT, WELL_SEPTIC_INSPECTION, WIND_HAIL_INSURANCE, LEASE_AGREEMENT, DIVORCE_DECREE. Doc types still without an entry continue to fall through to the generic prompt (no behaviour change for them). 339/339 unit tests green; live accuracy delta surfaces on the next `verify_pdf_extraction.py` run with the key set, or in production builds where the AI-Vision fallback fires.
- **PDF evidence is now real binary `.pdf`, not base64-encoded `.pdf.b64`.** Every channel that produces document evidence in `scripts/generate_realworld_simulation.py` writes raw PDF bytes (binary mode) so the files can be downloaded from S3 and opened directly. Touched: `email_inbox/`, `borrower_portal/`, `vendor_title/`, `shared_drive/`, plus the format-renderable siblings in `edms_pull/` and `los_encompass/`. `borrower_portal/` previously honoured a `format=jpg` extras hint by writing `.jpg.b64` — the contents were always a PDF, so the on-disk evidence is now consistently `.pdf` while `meta.original_filename` keeps the borrower's purported source extension (`drivers_license.jpg`) as descriptive metadata. `core/connectors/s3_connector.py`: `_EVIDENCE_SUFFIXES` simplified to `(".pdf", ".jpg", ".jpeg", ".png")`, the meta-pair `evidence_file` hint now points at the sibling `.pdf`. `scripts/verify_pdf_extraction.py` reads bytes directly (`pdf_path.read_bytes()`) instead of base64-decoding. Tests in `test_s3_connector_v2.py` write `.pdf` fixture bytes; new test renamed `test_sibling_pdf_files_are_ignored_by_dispatch`. After re-sync to S3 (`aws s3 sync … --sse AES256 --delete`): 0 stale `.pdf.b64` / `.jpg.b64` objects, 119 raw `.pdf` objects (page counts confirmed via `file(1)`: W-2 1pg, title 2-3pg, appraisal 4pg, BOA bank stmt 3pg). Prod build verified end-to-end: **`documents_pulled=196, documents_skipped=4, entities_updated=7, edges_created=648, read_failed=0`**.
...
```
3737719  fix(golden-record): pass real datetime to asyncpg, not ISO strings
06ffab9  feat(golden-record): backfill orchestrator + fix entity_states write-path
bdfa5bd  ops(ecs): track AWS rev 101 sizing in git — 1 vCPU / 4 GiB
65b776c  ops(ecs): scale task back to 0.5 vCPU / 1 GiB
8b1832c  fix(connector): cap pull at 200 docs per build + EOD marker
d8a6ba4  ops(ecs): bump task to 1 vCPU / 2 GiB
e3152ee  fix(catch-up): heartbeat + per-build error handling + force restart
e0bfb67  fix(catch-up): stop only on documents_pulled==0 AND apps==0
bad7d71  fix(catch-up): only stop when both documents AND applications are 0
4ba8e3a  fix(connector,scheduler): bound S3 pull to one date folder + async trigger
25991a6  fix(scheduler): persist catch-up state in Redis for multi-replica
69b2641  fix(scheduler): /scheduler/catch-up runs as background task
d392cc3  ops(ecs): point S3_SIMULATION_SOURCE at s3_scale_simulation
61eebfe  feat(v4.2): scale simulation — 22 profiles, catch-up, verification
ba5e749  feat(v3): two-stage connector dispatch + create_application_from_event
0e2b868  feat(simulator): v3 generator — full mortgage lifecycle, 50+ channels
6932566  feat(ops): wire ANTHROPIC_API_KEY + ENABLE_AI_EXTRACTION into ECS task
ce88fa6  feat(builder): AI-Vision classify shared_drive scans before los_id resolution
061a7d1  fix(extractor): inline error detail into ai_extraction_* warnings
ba89fa6  feat(extraction): expand AI-Vision _EXPECTED_FIELDS to 35 doc types
3423296  added Anthropic secret key
2fc36a1  docs: add binary-pdf phase row + tighten channel-connector PRD line
0245ce9  docs: capture .pdf-not-.pdf.b64 binary-evidence change
0cd79d0  feat(simulator): write real binary .pdf evidence (drop .pdf.b64)
75ac265  docs: refresh context.md + PRD with session 12's debug-logging + KMS fix
...
| **v3-pipeline** | `0e2b868`, `ba5e749` | **Full-lifecycle v3 simulator + two-stage connector dispatch + idempotent app creation.** `scripts/generate_realworld_simulation_v3.py` writes 522 files / 50+ channels / 28 active days for 10 loans across `{date}/loan_origination/` (the URLA event) and `{date}/post_application/{channel}/` (every other source: 4 LOS systems, 5 payroll providers, 9 vendors, 3 AMCs, 4 title companies, 6 insurance carriers, IRS/SSA/VA, closing agent, servicer (refi only), borrower portal + email + AI chat, HOA + condo project, conditions + corrections, compliance, LO notes, attorney legal). Every doc carries `source_document_id` in the originating system's native format (`ADP-W2-2025-4567`, `MERC-RPT-2026-1019`, `FA-TC-2026-78901`, `MISMO-001008`, etc.). Connector v3 detection in `_iter_channels` walks the two-stage layout; new format families: CSV (BytePro DictReader), XML (MISMO 3.4 ET local-name), application_event (loan_origination tagged `event_type='loan_application_submitted'`). Builder step 2.0 calls new `pg.create_application_from_event` (idempotent on los_id) before the los_id-resolution step. Every `upsert_entity_state` accumulates `legacy_ids` via JSONB `\|\|` merge (encompass IDs + every source_document_id seen this tick). `save_document` writes new `source_document_id` + `source_channel` columns. Schema additions strictly additive — destructive `scripts/reset_v3_data.sql` is a separate one-time recipe so ECS rolling deploys don't wipe prod data. 9 new v3 connector tests (344 -> 353 passing); v2 + v1 paths preserved (verified by 7 v2 tests). |
| **golden-record-backfill** | `06ffab9` → `3737719` | **Fix entity_states write-path mismatch + restartable backfill orchestrator.** `core/aggregation/entity_state_builder.py:upsert_all_entities` was calling `pg.upsert_entity_state(entity_id=, entity_type=, application_id=, state=, …)` against the actual signature `(application_id, state_data, tenant_id)` — every call landed in the per-entity `except Exception` clause; warnings the only signal. Result: 5 tables empty in prod (`entity_states`, `entity_state_events`, `income_profiles`, `credit_profiles`, `applicant_identity_xref`) despite 261k indexed docs. New module `core/aggregation/golden_record_builder.py` composes the row the schema wants — `rebuild_one(pg, redis, application_id, tenant_id)` runs credit + income assemblers, writes profiles (DELETE+INSERT, idempotent), upserts one identity xref per `(applicant_id, source_channel, source_document_id)` (UPSERT DO NOTHING), composes `state_data` with `borrower`/`co_borrowers`/`property`/`loan_terms` JSONB + indexed cols (mid_score, qualifying_monthly, combined_monthly_income, loan_amount, interest_rate with URLA→RATE_LOCK fallback, ltv, MI by LTV band, piti_monthly via amortization+tax+HOI+MI, dti_front/back, total_liquid_assets, 12-flag verifications, completeness_pct, status), UPSERTs via the correct signature, logs `entity_state_events`. `run_backfill(...)` streams `application_id ASC` via asyncpg server-side cursor (memory-safe on 9k apps), commits progress per-app to new singleton `golden_record_backfill_state` (last_completed_application_id / completed_count / total_count / status / errors / started_at / completed_at), resumes from cursor on restart, caps errors at 100. New endpoints `POST /admin/rebuild-golden-records?force=true` (202 + bg task) and `GET /admin/rebuild-golden-records/status` (live state + elapsed_seconds). Wired into `_run_assembly` via `_compose_and_upsert_entity_state` (skips re-running assemblers since the upload path already did) so future uploads keep all 5 tables current. Production crash on first run surfaced asyncpg quirk — ISO timestamp strings on TIMESTAMPTZ trigger `DataError: expected datetime, got 'str'`; fixed by storing real datetime objects in state + `_as_dt()` coercion helper at the write boundary. **359 → 362 green** (3 new in `test_golden_record_builder.py`: writes_to_all_five_tables, idempotent_on_re_run, skips_unknown_application). |
| **v4.4-cap** | `8b1832c` | **Per-build doc cap (200) + EOD marker + watermark stats surface.** Scale-corpus day folders carry 2k+ docs; loading them all into memory at once OOM'd Fargate even at 4 GiB. Connector now caps each pull at `_PULL_CAP_PER_BUILD = 200`. Picker changed `>` → `>=` so the bounded pull re-picks the same date until drained. When folder has more than cap → returns first 200 by `received_at`; wm lands mid-day; next build re-picks same folder. When folder has `<= cap` docs AND another folder exists ahead in window → connector appends in-band `{_eod_marker: True, received_at: start-of-next-day-iso}` doc; builder splits markers out before persist (no `document_id`) but folds `received_at` into `wm_to` so wm advances past drained folder. Builder also surfaces `watermark_from` + `watermark_to` on stats (these were missing entirely — that's why `current_watermark` was `null` in catch-up status). Catch-up break extended to `pulled == 0 && apps == 0 && !wm_advanced` (marker-only pull is still forward progress). 2000-doc day now drains in 10 builds instead of 1 that crashes. wm verified `2025-12-31` → `2026-01-21T17:15Z`. 3 new tests in `test_s3_connector_v2.py`. 356 → 359 green. |
| **v4-scale** | `61eebfe` → `d8a6ba4` | **9k-loan scale pipeline + bounded connector + zombie-hardened async catch-up.** `scripts/generate_scale_simulation.py` writes ~338k objects across 22 origination profiles (Conv/FHA/VA/USDA/Jumbo/NonQM/HELOC/Construction/Reverse/Bridge/Renovation/etc.) × 9000 loans × 60-day timeline with ±2-day per-loan jitter into `s3_scale_simulation/`; 100 loans get rendered binary `.pdf` evidence (the rest stay JSON-only). `scripts/verify_scale_results.py` runs 15 readiness checks (counts / watermark / edges / snapshots / completeness / `extraction_method` / contradicts) against any deployed API via `--api-url` + `--api-key`. Connector `_pull_sync` now bounded — sorts every date folder, picks ONLY the first strictly after `wm.date()`, returns `[]` when nothing's left; each build is a single calendar day's work so a fresh-from-epoch call doesn't ALB-timeout. `/scheduler/catch-up` and `/scheduler/trigger` both fire-and-forget background tasks; catch-up state lives at Redis key `catchup:state` so any replica's GET sees the same view (an in-process dict broke when POST landed on replica A and GET on replica B). Catch-up break condition evolved through the session: `docs_new==0` → `docs==0 && apps==0` → `pulled==0 && apps==0` (only the connector found absolutely nothing means truly idle). Zombie hardening: `last_heartbeat` written before AND after every build, POST handler auto-resets when `running + heartbeat>60s` stale, `?force=true` operator escape hatch, per-build try/except with `error_count>=3` → `stopped_reason='too_many_errors'`, `job_id` self-check inside bg loop so a force-race loser exits silently. POST accepts `max_builds` via JSON body; GET surfaces `heartbeat_age_s`. Cron collapsed to single `*/15 * * * *`. `/admin/reset` endpoint nukes prod data for re-runs. ECS task bumped to 1 vCPU / 2 GiB to halve per-build wall clock on the scale corpus. Live drain verified: watermark advanced `2025-12-31` → `2026-01-21T17:15Z`, `documents_pulled ~ 2 × documents_processed` (indexer correctly de-duplicating), `applications_created` accumulates from `loan_origination/` events. **356/356 tests green.** |
| **binary-pdf** | `0cd79d0` | **Real binary `.pdf` evidence (drop `.pdf.b64`).** Every channel that produces document evidence in `scripts/generate_realworld_simulation.py` now writes raw PDF bytes to `.pdf` files (binary mode) so they can be downloaded from S3 and opened directly — no client-side base64 decode required. Touched: `email_inbox/`, `borrower_portal/`, `vendor_title/`, `shared_drive/`, plus the format-renderable siblings in `edms_pull/` and `los_encompass/`. `borrower_portal`'s old `format=jpg` extras hint (which wrote `.jpg.b64` containing a PDF body anyway) is dropped from the on-disk extension; the borrower's purported source extension lives in `meta.original_filename` as descriptive metadata only. `core/connectors/s3_connector.py`: `_EVIDENCE_SUFFIXES` simplified to `(".pdf", ".jpg", ".jpeg", ".png")`; meta-pair `evidence_file` hint points at the sibling `.pdf`. `scripts/verify_pdf_extraction.py`: `pdf_path.read_bytes()` not `base64.b64decode(read_text())`; picker globs `*.pdf`. Test fixture writes binary stubs; new test renamed `test_sibling_pdf_files_are_ignored_by_dispatch`. After regen + S3 re-sync (`--sse AES256 --delete`): 0 stale `.pdf.b64`/`.jpg.b64` objects, 119 raw `.pdf` files; `file(1)` confirms PDF-1.4/1.3 + correct page counts (W-2 1pg, title 2–3pg, appraisal 4pg, BOA bank stmt 3pg). Prod build verified end-to-end against AWS task-def `:70`: `documents_pulled=196, documents_skipped=4, entities_updated=7, edges_created=648, read_failed=0`. **339/339 unit tests green** (no count change — replaced base64 decode in tests, didn't add new ones). |
